...
import type { ZombieManager } from '../utils/zombies';
import { getPointsManager } from '../utils/points';
import { getRoundManager, RoundManager } from '../utils/rounds';

export interface DebugOverlayActionsDeps {
  zombieManagerRef: React.MutableRefObject<ZombieManager | null>;
  setRoundState: Dispatch<SetStateAction<{ round: number; zombiesAlive: number; spawnStatus: string }>>;
  roundState: { round: number; zombiesAlive: number; spawnStatus: string };
}

interface DebugOverlayActionsResult {
  addPlayerPoints: (amount: number) => void;
  setPlayerPoints: (amount: number) => void;
  startDebugRound: (round: number) => void;
  goToNextDebugRound: () => void;
  goToPreviousDebugRound: () => void;
  forceEndDebugRound: () => void;
  spawnCurrentDebugWave: () => void;
  killAllDebugZombies: () => void;
  spawnDebugZombies: (count: number) => void;
}

/**
 * Hook that provides debug overlay action handlers for points, rounds, and zombies.
 * Extracted from App.tsx to reduce complexity.
 */
export function useDebugOverlayActions(deps: DebugOverlayActionsDeps): DebugOverlayActionsResult {
  const { zombieManagerRef, setRoundState, roundState } = deps;
